...
test_data = {
  ...
  for(var i = 1; i <= n; i++) {
    ...
    for(var j = 0; j < x_values.length; j++) {
      let x = x_values[j];
      let t = jStat.studentt.pdf(x, i);
      objects.push({value: x, density: t});
    }
...
var path = paths.append("path")
  ...
  path.each(function(d) { d.totalLength = this.getTotalLength(); })
    ...
    .transition()
    .duration(function(d, i){return (1-i/n)*(3000)})
    .delay(function(d, i){return  200*Math.sqrt(i*8)})
