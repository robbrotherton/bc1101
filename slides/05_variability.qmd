...
plot_histogram <- function(x, x_lab, y_lab = "Count", binwidth = 1) {
  require(ggplot2)
  
  ggplot() +
    geom_histogram(data = data.frame(x = x), aes(x), binwidth = binwidth, color = "black", fill = "grey60") +
    scale_x_continuous(breaks = min(x):max(x)) +
...
plot_norm <- function(xlim = c(-3, 3), ylim = c(0, .4), sd = 1) {
  require(ggplot2)
  
  ggplot() +
    stat_function(fun = dnorm, xlim = xlim, args = list(sd = sd)) +
    # scale_x_continuous(breaks = min(x):max(x)) +
    scale_y_continuous(expand = c(0,0), limits = ylim) +
    # labs(x = x_lab, y = y_lab) +
    theme_void() +
    theme(axis.line = element_line(),
          plot.margin = margin(l = 50, r = 10))
}

plot_save <- function(filename, width = 5, height = 2) {
  ggsave(here::here("slides", "media", filename), width = width, height = height, bg = "transparent")
}

plot_histogram(you, x_lab = "Your scores")
plot_save("you.dist.svg")
plot_histogram(friend, x_lab = "Friend")
plot_save("friend-dist.svg")
