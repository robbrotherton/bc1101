...
However, as part of describing and visualizing the data I will ask you to compute two different confidence intervals: one for each group of scores as you have done previously. (Hint: remember `summarize()` with its `.by` argument applied to long-format data.) Note that those individual group confidence intervals quantify something different from the one produced by the `t.test()` function in this case: the margin of error associated with each individual sample's point estimate.
