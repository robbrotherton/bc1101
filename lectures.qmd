...
recitation <- seq.Date(from = as.Date("2025-01-22"), to = as.Date("2025-05-05"), by = "week")

info <- info |> 
  dplyr::filter(stringr::str_detect(Topic, ":"))

dates <- info |> dplyr::pull(Date)
links <- info |> dplyr::pull(Panopto)
```
...
```{r}
card("09_hypothesis-testing", dates[9], "invertable",
     panopto_link = links[9])
```
::::
...
```{r}
card("10_hypothesis-testing-pt-2", dates[10], "invertable-blue",
     panopto_link = links[10])
```
::::
...
```{r}
card("12_the-t-test-pt-2", dates[12], "invertable-bw",
...
```{r}
card("13_independent-samples-t-test", dates[13],
...
```{r}
card("14_related-samples-t-test", dates[14],
...
```{r}
card("17_related-samples-ANOVA", dates[17],
     panopto_link = links[17])
```
::::
...
## 18. Factorial ANOVA
```{r}
card("18_factorial-ANOVA", dates[18],
     panopto_link = links[18])
```
::::
...
## 19. Correlation
```{r}
card("19_correlation", dates[19], image_class = "invertable",
     panopto_link = links[19])
```
::::
...
## 20. Regression
```{r}
card("20_regression", dates[20], image_class = "invertable",
     panopto_link = links[20])
```
::::
