...
The Quarto document supports regular text, as well as code and output. The first thing I want you to do is just practice typing some plain text and get used to the editor interface. For the first couple of question, delete the placeholder text and type your own answers on a new line. Notice how the visual editor allow you to change the text format--you can make text bold or italicized, you can have different heading levels, lists, and so on. You might not need all these tools right away, but it's nice to know they're there.
...
::: {.content-visible when-format="html"}
The last to introduce in this first problem set is the idea of working with external data files. If a researcher were working with actual data, it would most often be in the form of a separate file, such as a .csv (for comma separated values). You can open a .csv in Excel or Google Sheets and you should see a spreadsheet with columns and rows.

To read a .csv file into R, we use the `read.csv()`function, specifying the path to the file within the parentheses. There's a file in the "data" folder of the Problem Sets posit.cloud project named `1_example.csv`. We can read it into R like this:

```{r}
read.csv("data/1_example.csv")
```

That line of code produces a column of numbers in the console and/or on your rendered pdf. Notice, however, that although we can see the scores, they aren't "saved" for us to work with in R. To do that, we need to assign the result to a name, like so:
