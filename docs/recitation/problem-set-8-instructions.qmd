...
However, `TukeyHSD()` also only works with between-participants models; for a within-participants model you'll need to use the mathematical approach from the lectures, using $MS_{error}$ and $df_{error}$ from the denominator of the related-samples $F$-ratio. You will find these values in the output of the `summary()` function.
...
…where `p` = alpha, `nmeans` = $k$, and `df` = $df_{denominator}$. The resulting $q$ value should be used in the calculation.

$$q * \sqrt{\dfrac{MS_{denominator}}{n}}$$

This tells you the size of difference between means required to count as statistically significant.


## Part 2: Loftus & Palmer replication.

In the previous problem set you analyzed and visualized data from a two-group study design. In this part, you'll do the same thing for data that has three groups. The nice thing about the `dplyr::summarize()` approach to computing summary statistics is that it doesn't matter how many groups you have; the code should work almost identically. Supplying the grouping variable via `summarize()`'s `.by` argument does all the work. So if you're feeling stuck, make sure to revisit your own previous work.
