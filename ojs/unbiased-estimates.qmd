...
const timeChartHorizontal = {
  ...
  function setUpTimeAxes(vertical = true) {
  ...
  if (vertical) {
    ...
    timeXAxis = d3.axisTop(timeX).ticks(5).tickSize(-timeChart.height - (timeChart.margin.top + timeChart.margin.bottom))
    timeYAxis = d3.axisRight(timeY).tickSize(0)
  } else {
  timeY = d3.scaleLinear()
      .domain([-0.5, 0.5])
      .range([timeChartHorizontal.height - timeChartHorizontal.margin.bottom, timeChart.margin.top])
      timeX = d3.scaleLinear()
      .domain([0, 200])
      .range([timeChartHorizontal.margin.left, timeChartHorizontal.width - timeChartHorizontal.margin.right])
    biasLine = function(x, y){
        return d3.line()
        .x(function(d,i) { return timeX(y[i]); })
        .y(function(d,i) { return timeY(x[i]); })
        (Array(x.length));
    }
    timeXAxis = d3.axisBottom(timeX).tickSize(0)
    timeYAxis = d3.axisLeft(timeY).ticks(5).tickSize(-timeChartHorizontal.width - timeChartHorizontal.margin.left - timeChartHorizontal.margin.right)
  }
}
  ...
  setUpTimeAxes();
  const timeSvg = makeTimeseriesChart(timeChart);
  
  const timeseriesData = timeSvg.append("g");
  ...
  function positionTimeAxes(vertical = true) {
    if (vertical) {
      timeChartXAxis.attr("transform", `translate(0, ${timeY(0)})`)

      timeChartXAxis.append("rect")
        .attr("width", timeChart.width)
        .attr("y", -timeChart.margin.top)
        .attr("height", timeChart.margin.top)
        .attr("rx", 5)
        .attr("fill", "var(--timeseries-panel-background)")
      ...
      timeChartXAxis.attr("transform", `translate(0, ${timeChartHorizontal.height - timeChartHorizontal.margin.bottom})`)

      timeChartXAxis.append("rect")
        .attr("width", timeChartHorizontal.margin.left)
        .attr("height", timeChartHorizontal.height)
        .attr("rx", 5)
        .attr("fill", "var(--timeseries-panel-background)")
      
          timeChartYAxis.attr("transform", `translate(${timeChartHorizontal.margin.left}, 0)`)
  ...
  function updatePath() {
      timeseriesData.selectAll("g").remove()
...
function clearData() {
    ...
    sampleEstimatesTemp.selectAll("path").remove()
    timeseriesData.selectAll("path").remove()
  }
