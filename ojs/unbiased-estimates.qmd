...
::: {#population-container style="float: left; width: 60%;"}
:::

::: {#estimates-container style="float: right; width: 40%;"}
:::

::: {#timeline-container style="padding-top: 1em; width: 100%;"}
:::
...
populationPanelHeight = 400;

populationSubPanelProportion = 0.7;
sampleSubPanelProportion = 0.1;
estimatesSubPanelProportion = 0.2;
...
yScalePopulation = d3.scaleLinear()
    .domain([0, 300])
    .range([populationPanelHeight * 0.7, 0])
...
yScaleEstimates = d3.scaleLinear()
    .domain([20, 0])
    .range([estimatesPanelHeight - 75, 50])
    
yPlacementEstimatesSubPanel = d3.scaleLinear()
  .domain([0, 2])
  .range([20 + populationPanelHeight * (1 - estimatesSubPanelProportion), populationPanelHeight - 20])
...
update_svg = {
  ...
  if (playing) durationMultiplier = 1;
    ...
    sampleCircles.selectAll('circle')
      ...
      // show estimates
    let currentEstimates = sample_estimates.flat().slice(-3);
    
    for (let i = 0; i < 3; i++) {
      if (currentEstimates[i].param === "mean") {
      currentEstimates[i].originalValue = currentEstimates[i].value;
      currentEstimates[i].cy = yPlacementEstimatesSubPanel(0);
      }
      if (currentEstimates[i].param === "population") {
      currentEstimates[i].originalValue = currentEstimates[i].value + 1;
      currentEstimates[i].cy = yPlacementEstimatesSubPanel(1);
      }
      if (currentEstimates[i].param === "sample") {
      currentEstimates[i].originalValue = currentEstimates[i].value + 1;
      currentEstimates[i].cy = yPlacementEstimatesSubPanel(2);
      }
    }
    
    sampleEstimates.selectAll('path').remove()
    sampleEstimates.selectAll('path')
      .data(currentEstimates)
      .enter().append("path")
      .attr("id", d => d.param + "-estimate")
      .attr("d", d3.symbol().type(d3.symbolSquare).size(radius * radius * 2))
      .attr("transform", d => `translate(${xScalePopulation(d.originalValue)}, ${d.cy}) rotate(45)`)
      .attr("fill", "black")
  }
  ...
  populationAndSampleSvg.append("text")
    .text("Sample")
    .attr("y", populationPanelHeight * populationSubPanelProportion + 20)
  populationAndSampleSvg.append("text")
    .text("Estimates")
    .attr("y", populationPanelHeight * (1 - estimatesSubPanelProportion) + 20)
  ...
  // show parameters
  const parameterPlacement = [{param: "mu", x: 0, y: yPlacementEstimatesSubPanel(0), text: "μ"},
  {param: "sigma", x: 1, y: yPlacementEstimatesSubPanel(1.5), text: "σ"}]
  
  parameters.selectAll("path").data(parameterPlacement).enter().append("path")
    .attr("d", d3.symbol().type(d3.symbolSquare).size(radius * radius * 4))
    .attr("transform", d => `translate(${xScalePopulation(d.x)}, ${d.y})`)
      .attr("stroke", d => "var(--" + d.param + "-color")
      .attr("fill", "none")
  parameters.selectAll("text").data(parameterPlacement).enter().append("text")
    .attr("transform", d => `translate(${xScalePopulation(d.x) + 5}, ${d.y + 5})`)
    .append("tspan")
    .text(d => d.text)
...
var timeYAxis = d3.axisLeft(timeY).ticks(5).tickSize(0)
